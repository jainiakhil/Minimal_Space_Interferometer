...
#    plt.scatter(arrbu12, arrbv12, color = 'r', marker = 'o', s = 0.1, label = 'Baseline between Satellite 1 and Satellite 2')
#    plt.scatter(arrbu23, arrbv23, color = 'b', marker = 'o', s = 0.1, label = 'Baseline between Satellite 2 and Satellite 3')
#    plt.scatter(arrbu31, arrbv31, color = 'g', marker = 'o', s = 0.1, label = 'Baseline between Satellite 3 and Satellite 1')
...
#    np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] u-v Array %d' % (sst, save), (arrbu12, arrbv12, arrbu23, arrbv23, arrbu31, arrbv31))
...
def generatematplot(evt):
    ...
    plt.grid(True)
    global save
    plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] u-v Plot with Hermitian for %d days for source at %0.0f hr and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
    np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] u-v Array for %d days for source at %0.0f hr %0.0f deg' % (sst, day, nsrahr, nsdecdeg), (arrbu12, arrbv12, arrbu23, arrbv23, arrbu31, arrbv31))
    np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] Source Info for %d days for source at %0.0f hr %0.0f deg' % (sst, day, nsrahr, nsdecdeg), (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec))
    save += 1
...
def pltcovergrid(evt):
    global arrbu12, arrbu23, arrbu31, arrbv12, arrbv23, arrbv31
    size12 = len(arrbu12)
    size23 = len(arrbu23)
    size31 = len(arrbu31)
...
##Defining the grid as per the cell size for the plot due to satellites 1 and 2
    ...
    plt.tick_params(labelsize = 14)
    gridfig1 = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Coverage due to Sat 1 and Sat 2 - Uniformly Weighted for %d days for source at %0.0f h and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
...
##Defining the grid as per the cell size for the plot due to satellites 2 and 3
    ...
    gridfig2 = plt.title('u-v Grid of Source %d at RA: %0.0f h %0.0f m %0.0f s and Dec: %0.0f deg %0.0f min %0.0f sec' % (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec), fontsize = 13)
    plt.xlim([0,300])
    plt.ylim([0,300])
    xlocs, xlabels = plt.xticks()
    xlabels = [(int(((item)*105)-15750)) for item in xlocs]
    ...
    ylabels = [(int(((item)*105)-15750)) for item in ylocs]
    plt.yticks(ylocs, ylabels)
    gridfig2 = plt.xlabel('u (km)', fontsize = 13)
    gridfig2 = plt.ylabel('v (km)', fontsize = 13)
    plt.tick_params(labelsize = 14)
    gridfig2 = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Coverage due to Sat 2 and Sat 3 - Uniformly Weighted for %d days for source at %0.0f h and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')

##Defining the grid as per the cell size for the plot due to satellites 3 and 1
    grid = np.zeros(((31500/cell), (31500/cell)), dtype = int)

    for i in range(0, size31):
        k = int(math.floor((arrbu31[i]/cell))) + (15750/cell)
        l = int(math.floor((arrbv31[i]/cell))) + (15750/cell)
        if grid[l][k] == 0:
            grid[l][k] = 1
            gridcount3 += 1
        else:
    ...
    gridfig3 = plt.title('u-v Grid of Source %d at RA: %0.0f h %0.0f m %0.0f s and Dec: %0.0f deg %0.0f min %0.0f sec' % (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec), fontsize = 13)
    plt.xlim([0,300])
    plt.ylim([0,300])
    xlocs, xlabels = plt.xticks()
    xlabels = [(int(((item)*105)-15750)) for item in xlocs]
    ...
    ylabels = [(int(((item)*105)-15750)) for item in ylocs]
    plt.yticks(ylocs, ylabels)
    gridfig3 = plt.xlabel('u (km)', fontsize = 13)
    gridfig3 = plt.ylabel('v (km)', fontsize = 13)
    plt.tick_params(labelsize = 14)
    gridfig3 = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Coverage due to Sat 3 and Sat 1 - Uniformly Weighted for %d days for source at %0.0f h and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
...
##Defining the grid as per the cell size for the plot due to all the satellites
    ...
    gridfig4 = plt.title('u-v Grid of Source %d at RA: %0.0f h %0.0f m %0.0f s and Dec: %0.0f deg %0.0f min %0.0f sec' % (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec), fontsize = 13)
    plt.xlim([0,300])
    plt.ylim([0,300])
    xlocs, xlabels = plt.xticks()
    xlabels = [(int(((item)*105)-15750)) for item in xlocs]
    ...
    ylabels = [(int(((item)*105)-15750)) for item in ylocs]
    plt.yticks(ylocs, ylabels)
    gridfig4 = plt.xlabel('u (km)', fontsize = 13)
    gridfig4 = plt.ylabel('v (km)', fontsize = 13)
    plt.tick_params(labelsize = 14)
    gridfig4 = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Coverage Total - Uniformly Weighted for %d days for source at %0.0f h and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
...
##Doing 2D IFFT on the grid due to all the satellites to get the dirty beam 
    ...
    fftimg = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Dirty Beam - Uniformly Weighted for %d days for source at %0.0f h and %0.0f deg.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
    #print(fftgrid)


##Plotting Percentage Coverage vs Observation Duration
    global arrday, arrcov
    coverageplot = plt.figure(figsize = (10,10))
    coverageplot = plt.title('Percentage Coverage vs Observation Duration', fontsize = 13)
    coverageplot = plt.xlabel('Time (in Days)', fontsize = 13)
    coverageplot = plt.ylabel('Percentage Coverage', fontsize = 13)
    coverageplot = plt.grid(True)
    coverageplot = plt.plot(arrday, arrcov, color='black', marker='o', linestyle='solid', linewidth=2, markersize=6)
    coverageplot = plt.savefig('D:\Space Interferometer Project\Tests\PDF Plots from Live Run\[%s] Coverage vs Time Plot for %d days for source at %0.0f h and %0.0f deg.pdf.pdf' % (sst, day, nsrahr, nsdecdeg), bbox_inches = 'tight')
    print(arrday, arrcov)
...
sat1phase = (00)*(pi/180)                   ##Initial phase of satellite 1
sat2phase = (90)*(pi/180)                   ##Initial phase of satellite 2
sat3phase = (90)*(pi/180)                   ##Initial phase of satellite 3

sat1dist = 770                              ##Distance above earth surface in km for satellite 1
sat2dist = 1085                              ##Distance above earth surface in km for satellite 2
...
sat1 = sphere(pos = vector(sat1x,0,sat1z), radius = 100, color = color.red, make_trail = True, retain = 2000, trail_type = "curve")                           ##Equitorial satellite at a height of 429 km
sat2 = sphere(pos = vector(sat2x,sat2y,0), radius = 100, color = color.blue, make_trail = True, retain = 2000, trail_type = "curve")                          ##Polar satellite at a height of 729 km
...
sat1axis = cylinder(pos = vector(0,-7000,0), radius = 50, length = 14000, axis = vector(0,1,0), color = sat1.color, center = earth.pos)
...
sourcedivv = nsourcedist/3e4                                                                                                                      ##Generating random constant to act as divisor
...
print("[%s] \nRunning the program: Test with uniformly weighted grid for coverage and dirty beam with Hermitian nature with 10 second plotting including Matplotlib saving arrays in binary without fixed celestial sphere and with constantly updating source and avoiding ionospheric effects and saving u-v plots every day by removing the effects due to the sun \n\nSatellite 1 is at a height of %0.1f km, having an initial speed of %0.3f km/s and a period of %0.3f s \nSatellite 2 is at a height of %0.1f km, having an initial speed of %0.3f km/s and a period of %0.3f s \nSatellite 3 is at a height of %0.1f km, having an initial speed of %0.3f km/s and a period of %0.3f s \n" % (sst, sat1dist, sat1.speed, sat1.period, sat2dist, sat2.speed, sat2.period, sat3dist, sat3.speed, sat3.period))
...
save = 0                ##To save plot number
percov = 0              ##To save percent coverage
gridcounttot1 = 0       ##Coverage grid
#tsat1sun = 0
#tsat2sun = 0
#tsat3sun = 0
...
##    
##    earthaxis.pos = sat1axis.pos = earth.pos + vector(0,-7000,0)                          ##Update position of earth and satellite 1 axes (since they overlap)
...
#    Fa1s = ((G*sun.mass)/r1s**2)*rv1s                                                     ##Acceleration due to gravitaitonal force of sun
    Fa1net = Fa1                                                                   ##Net acceleration due to gravitational force
    ...
    sat1field.length = 1000                                                               ##Thickness of satellite FoV. Just for the purpose of understanding the perspective
...
#    Fa2s = ((G*sun.mass)/r2s**2)*rv2s                                                     ##Acceleration due to gravitaitonal force of sun
    Fa2net = Fa2                                                                   ##Net acceleration due to gravitational force
    ...
    sat2field.length = 600                                                                ##Thickness of satellite FoV. Just for the purpose of understanding the perspective
...
#    Fa3s = ((G*sun.mass)/r3s**2)*rv3s                                                     ##Acceleration due to gravitaitonal force of sun
    Fa3net = Fa3                                                                   ##Net acceleration due to gravitational force
    ...
    sat3field.length = 50                                                                 ##Thickness of satellite FoV. Just for the purpose of understanding the perspective
#    f3.plot(pos = (t, mag(sat3.v - earth.v)))
    ...
    if (sat2.pos == sat3.pos):
        ...
        break
    if (sat3.pos == sat1.pos):
        print("[%s] \nSatellite 3 has crashed with Satellite 1 at time %d seconds!" % (sst, t))
        break
    if (re <= sun.radius):
...
#        plt.scatter(arrbu23, arrbv23, color = 'b', marker = 'o', s = 0.1, label = 'Baseline between Satellite 2 and Satellite 3')
#        plt.scatter(arrbu31, arrbv31, color = 'g', marker = 'o', s = 0.1, label = 'Baseline between Satellite 3 and Satellite 1')
#        plt.axis([-14000, 14000, -14000, 14000])
#        plt.title('u-v Plot of Source %d at RA: %0.0f h %0.0f m %0.3f s and Dec: %0.0f deg %0.0f min %0.3f sec' % (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec))
#        plt.xlabel('u')
#        plt.ylabel('v')
#        #plt.legend()
#        plt.grid(True)
#        plt.savefig('D:\Space Interferometer Project\Tests\EPS Plots from Live Run\[%s] u-v Plot %d.pdf' % (sst, save), bbox_inches = 'tight')
#        np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] u-v Array %d' % (sst, save), (arrbu12, arrbv12, arrbu23, arrbv23, arrbu31, arrbv31))
#        np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] Source Info %d' % (sst, save), (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec))
#        save += 1
...
##Recreating graph display to plot the u-v plots for all baselines whenever a new source is set
        ...
        fb12 = gdots(color = color.red, size = 0.1)
        fb23 = gdots(color = color.blue, size = 0.1)
        fb31 = gdots(color = color.green, size = 0.1)
...
##If the source is within the FoV of all three satellites, then tracking is started (Tracking measurement is commented out for now)
        if ((sat1vis > 0) and (sat2vis > 0) and (sat3vis > 0)):
            #track += 1
            #print(track)
            ...
            bu31 = np.dot((sat3.pos - sat1.pos),uunitvec)
            bv31 = np.dot((sat3.pos - sat1.pos),vunitvec)
            arrbu31.append(bu31)
            arrbu31.append(-bu31)
            arrbv31.append(bv31)
            arrbv31.append(-bv31)
        ...
        elif ((sat1vis > 0) and (sat2vis > 0) and (sat3vis < 0)):
...
##To plot u-v graph every day (comment out when not required to improve performance)
##        if (day%5 == 0):
        plt.figure(figsize = (10,10))
        plt.scatter(arrbu12, arrbv12, color = 'r', marker = 'o', s = 0.1, label = 'Baseline between Satellite 1 and Satellite 2')
        plt.scatter(arrbu23, arrbv23, color = 'b', marker = 'o', s = 0.1, label = 'Baseline between Satellite 2 and Satellite 3')
        ...
        np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] u-v Array for %d days for source at %0.0f hr %0.0f deg' % (sst, day, nsrahr, nsdecdeg), (arrbu12, arrbv12, arrbu23, arrbv23, arrbu31, arrbv31))
        np.save('D:\Space Interferometer Project\Tests\Binary Files\[%s] Source Info for %d days for source at %0.0f hr %0.0f deg' % (sst, day, nsrahr, nsdecdeg), (nsrc, nsrahr, nsramin, nsrasec, nsdecdeg, nsdecmin, nsdecsec))
        print("Generating u-v plot...")
        save += 1

##To plot percentage coverage vs time graph
        size12 = len(arrbu12)
        size23 = len(arrbu23)
        size31 = len(arrbu31)
        cell = 105
        grid1 = np.zeros(((31500/cell), (31500/cell)), dtype = int)
        ...
        percov = gridcounttot1/706.85
        arrday.append(day)
        arrcov.append(percov)
        #print(arrday, arrcov)
        gridcounttot1 = 0
